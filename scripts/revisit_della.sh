#!/bin/bash
#SBATCH --job-name=spline       
#SBATCH --output=logs/%x_%j.out
#SBATCH --error=logs/%x_%j.out
#SBATCH --gres=gpu:1
#SBATCH --mem=32G
#SBATCH --time=59:00
#SBATCH --cpus-per-task=4

# quick_run.sh — Interactive quick-start for testing on a GPU node
#
# Usage:
#   srun --partition=gpu --gres=gpu:1 --mem=32G --time=02:00:00 --pty bash
#   conda activate spline_fields
#   bash quick_run.sh [model_name]
#
# Examples:
#   bash quick_run.sh straight     # fast, smallest model
#   bash quick_run.sh wCurly       # challenging curly hair
#   bash quick_run.sh wWavy        # wavy hair


# load modules or conda environments here
source ~/.bashrc
eval "$(conda shell.bash hook)"  # this is needed to load python packages correctly

conda activate /scratch/gpfs/CHIJ/christina/envs/spline_fields
module load cudatoolkit/11.8
MODEL="wWavy"
# MODEL=${1:-straight}
echo "============================================================"
echo "  Quick Run: ${MODEL}"
echo "  GPU: $(nvidia-smi --query-gpu=name --format=csv,noheader 2>/dev/null || echo 'unknown')"
echo "============================================================"

# Download if needed
# python -c "from hair_loader import download_yuksel_hair; download_yuksel_hair('${MODEL}')"

# Run v2_full
# python run_full_baseline_pipeline.py \
#   --model-name wWavy \
#   --device cuda \
#   --output-root outputs/with_gs/full_pipeline_wWavy \
#   --external-spline-eval-samples 12 \
#   --num-curves 500 \
#   --K 12 \
#   --pc-points-per-curve 12 \
#   --gs-points-per-curve 12 \
# Robust comparison (multi-seed + multi held-out + spline/point/Gaussian):
#   --seeds 42,43,44,45,46 \
#   --held-out-azimuths 45,135,225,315 \
python experiment_revisit_memory.py \
  --model-name wWavyThin \
  --output-dir outputs/revisit/revisit_memory_wWavyThin \
  --device cuda  \
  --seeds 42,43,44,45,46 \
  --held-out-azimuths 45,135,225,315 \
  --poster-seed 42


# python evaluate_external_targets.py \
#   --spline-results outputs/my_dense_run/opt_results.pt \
#   --point-results outputs/pointcloud_baseline/point_baseline_results.pt \
#   --model-name wStraight \
#   --data-dir data/hairmodels \
#   --output-dir outputs/external_eval \
#   --bin-size 0 \
#   --device cuda

# python run_pointcloud_baseline.py --model-name wStraight --device cuda --output-dir outputs/pointcloud_baseline


# python run_dense.py --model-name wStraight --output-dir outputs/my_dense_run --device cuda
# python optimize_v2.py \
#     --data-source yuksel \
#     --model-name ${MODEL} \
#     --num-curves 50 \
#     --K 8 \
#     --num-views 36 \
#     --steps-per-view 80 \
#     --reproj-weight 2.0 \
#     --curv-weight 0.1 \
#     --tangent-weight 0.05 \
#     --anchor-weight 0.01 \
#     --view-buffer 3 \
#     --ema-decay 0.7 \
#     --output-dir outputs/quick_${MODEL}

echo ""
echo "  Done! See outputs/revisit/revisit_memory_wWavyThin/revisit_results.json"
echo "  Poster: outputs/revisit/revisit_memory_wWavyThin/revisit_poster.png"
