...
STEPS_PER_VIEW = 120
SEED = 0
OUTPUT_PLOT_PATH = "curve_memory_demo_plots_vs_baseline.png"

# Regularization strength for structured curve memory
SMOOTHNESS_WEIGHT = 0.05

# Number of free points for unstructured baseline
NUM_BASELINE_POINTS = NUM_SAMPLES
...
def render_point_view(points: torch.Tensor, angle_deg: float) -> torch.Tensor:
    """
    Rotate free points and render them directly.
    """
...
def smoothness_loss(ctrl: torch.Tensor) -> torch.Tensor:
    """
    Encourage neighboring control points to stay close / smooth.
    """
...
def point_set_drift(curr_pts: torch.Tensor, gt_pts: torch.Tensor) -> torch.Tensor:
    """
    Mean Euclidean distance between corresponding baseline points.
    Assumes initialization comes from sampled GT ordering.
    """
...
def optimize_sequential_views(init_ctrl: torch.Tensor, gt_ctrl: torch.Tensor, view_angles):
    ...
    for view_idx, angle in enumerate(view_angles):
        ...
        with torch.no_grad():
            ...
            print(
                f"[Curve] View {view_idx:02d} | angle={angle:6.1f} | "
...
def optimize_sequential_point_baseline(init_points: torch.Tensor, gt_points: torch.Tensor, view_angles):
    """
    Baseline: optimize an unstructured set of free points directly.
    """
    ...
    for view_idx, angle in enumerate(view_angles):
        target = render_point_view(gt_points, angle).detach()
        ...
        with torch.no_grad():
            ...
            print(
                f"[Points] View {view_idx:02d} | angle={angle:6.1f} | "
                f"final_loss={final_loss:.6f} | drift={drift:.4f} | revisit_err={revisit_error:.6f}"
            )
...
def plot_points(ax, points, title="", color="tab:orange"):
    ...
    ax.scatter(pts[:, 0], pts[:, 1], color=color, s=8)
    ax.set_xlim(-1, 1)
    ax.set_ylim(-1, 1)
    ax.set_aspect("equal")
    ax.set_title(title)
...
def main():
    ...
    # Ground-truth sampled points for baseline
    gt_points = sample_polyline(gt_ctrl, NUM_BASELINE_POINTS).detach()

    # Noisy point baseline initialization
    init_points = gt_points + 0.18 * torch.randn_like(gt_points)

    print(f"Using device: {DEVICE}")

    # --------------------------------------------------------
    # Part 1: sanity check on a single view (structured memory)
    # --------------------------------------------------------
    single_angle = 25.0
    ...
    # --------------------------------------------------------
    # Part 2: sequential structured-memory experiment
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Part 3: sequential unstructured-point baseline
    # --------------------------------------------------------
    ...
    # Row 1: structured memory geometry
    ...
    # Row 2: baseline geometry / renders
    ...
    ax14.plot(curve_results["drift_history"], marker="o", label="Curve + Smoothness")
    ax14.plot(point_results["drift_history"], marker="o", label="Unstructured Points")
    ...
    ax15.plot(curve_results["revisit_error_history"], marker="o", label="Curve + Smoothness")
    ax15.plot(point_results["revisit_error_history"], marker="o", label="Unstructured Points")
    ...
    ax16.plot(curve_results["per_step_loss"], label="Curve + Smoothness")
    ax16.plot(point_results["per_step_loss"], label="Unstructured Points")
